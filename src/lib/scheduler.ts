...
let sslCheckIntervalId: ReturnType<typeof setInterval> | null = null;
...
export function startScheduler() {
  ...
  // Initial delays to stagger startup
  ...
  console.log(`[scheduler] Started — checks every ${CHECK_INTERVAL_MS / 1000}s, cleanup & SSL check every 24h`);
}

export function stopScheduler() {
  if (checkIntervalId) clearInterval(checkIntervalId);
  if (cleanupIntervalId) clearInterval(cleanupIntervalId);
  if (sslCheckIntervalId) clearInterval(sslCheckIntervalId);
  checkIntervalId = null;
  cleanupIntervalId = null;
  sslCheckIntervalId = null;
